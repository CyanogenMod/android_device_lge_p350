...
static uint32_t SND_DEVICE_TTY_HCO_SPEAKER=-1;
static uint32_t SND_DEVICE_HANDSET_VR=-1;
static uint32_t SND_DEVICE_HEADSET_VR=-1;
...
static uint32_t SND_DEVICE_HEADSET_LOOPBACK=-1;
static uint32_t SND_DEVICE_HANDSET_LOOPBACK=-1;
// ----------------------------------------------------------------------------
    ...
    if (m7xsnddriverfd >= 0) {
        ...
        if (rc >= 0) {
            ...
            for (int cnt = 0; cnt < mNumSndEndpoints; cnt++, ept++) {
                ...
                LOGE("cnt = %d ept->name = %s ept->id = %d\n", cnt, ept->name, ept->id);
	      #define CHECK_FOR(desc) \
	      if (!strcmp(ept->name, #desc)) { \
		      SND_DEVICE_##desc = ept->id; \
		      LOGD("BT MATCH " #desc); \
		}
		CHECK_FOR(HANDSET_LOOPBACK);
		CHECK_FOR(HANDSET);
		CHECK_FOR(HEADSET_LOOPBACK);
		CHECK_FOR(HEADSET);
		...
		CHECK_FOR(TTY_VCO);
		CHECK_FOR(TTY_HCO);
		CHECK_FOR(TTY_HCO_SPEAKER);
		CHECK_FOR(HANDSET_VR);
		CHECK_FOR(HEADSET_VR);
		CHECK_FOR(BT_VR);
		CHECK_FOR(CURRENT);
		  #undef CHECK_FOR
            }
